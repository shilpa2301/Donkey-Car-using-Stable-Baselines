...
class AutoencoderWrapper(gym.Wrapper):
    ...
    def __init__(self, env: gym.Env, ae_path: Optional[str] = os.environ.get("AAE_PATH")):  # noqa: B008
        ...
        self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(self.ae.z_size +2,), dtype=np.float32)
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict[str, Any]]:
        ...
        # encoded_image = self.ae.encode_from_raw_image(obs[:,:,::-1])
        steer = infos["steer"]
        throttle = infos["throttle"]
        # steer = infos["steering"]
        # print("========== speed=",speed)
        #shilpa donkey car
        new_obs = np.concatenate([obs.flatten(), [float(throttle), float(steer)]])
